...
int main(){
    ...
    char* all_files[100] = {"USA-road-d.BAY.gr","USA-road-d.CAL.gr","USA-road-d.COL.gr","USA-road-d.FLA.gr","USA-road-d.LKS.gr","USA-road-d.NE.gr","USA-road-d.NW.gr","USA-road-d.NY.gr"};
    double time[9];
    for(int i = 0 ; i < 8; i++){
        ...
        while(query_number-- > 0) {
            //生成随机查询对
            src = rand() % (num_edges/3); // 随机生成源节点(利用边数估计顶点数)
            dest = rand() % (num_edges/3); // 随机生成目标节点
            // 调用算法查询最短路径
            int distance = query(graph, src, dest);
        }
        clock_t end_time = clock();
        time_cost = (double)(end_time - start_time) / CLOCKS_PER_SEC;
        printf("%s: time cost is: %f s\n\n", file_name,time_cost);
        time[i] = time_cost;
        free(graph);
        free(heap);
    }
    ...
    for(int i = 0; i < 8; i++)
        printf("The time cost of %s is: %f s\n", all_files[i], time[i]);
}
